...
public void Close()
{
	...
	{
		PerformFlush();
		RemoveBlankLines(m_FileName); //remove double blank lines after every close
		//Clean up memory
		m_FileName = null;
		m_iniString = null;
	}
...
/// <summary>
/// return a float value from a key
/// </summary>
...
/// <summary>
/// remove extra blank lines in the file
/// </summary>
public void RemoveBlankLines(string path)
{
	int blanklines=0;
	var tempFileName = path + "temp";//Path.GetTempFileName();
	try
	{
			...
			using (var streamWriter = new StreamWriter(tempFileName))
			{
				...
				while ((line = streamReader.ReadLine()) != null)
				{
					...
					if (!string.IsNullOrWhiteSpace(line))
					{
						...
						blanklines=0;
						//if (line.Split(new[] { "\r\n", "\r", "\n" },StringSplitOptions.None).Length > 1) Debug.Log("new line encountered"); //attempting to remove the last trailing newline
					}
					if (string.IsNullOrWhiteSpace(line))
					{
						//if (line.Split(new[] { "\r\n", "\r", "\n" },StringSplitOptions.None).Length > 1) Debug.Log("new line encountered"); //attempting to remove the last trailing newline
						...
						if (blanklines==2)
						{
							//Debug.Log("Found two blank lines.");
							blanklines=0;
						}
						else streamWriter.WriteLine(line); //write one blank line every time 2 lines are encountered.
					}
